                           ...
                           unsigned long long timestamp_ms)
{
    ...
    int r;
    uint8_t static packet_id = 0;
    uint8_t static final_data[1500] = {0};
    uint16_t static final_data_len = 0;
    ...
    if(src_ep == 10 && dst_ep == 10) {
	// Non Fragment packet
	if(bytes[1] == NON_FRAG) {
		final_data_len = num_bytes - FRAGMENT_HEADER_SIZE;
		memcpy(final_data, bytes + FRAGMENT_HEADER_SIZE, final_data_len);
		LOGI(" final data len : %d\n", final_data_len);
	}
	// Assemble fragments
	else {
	    if(bytes[1] != END_FRAG) { //Not an end fragment
		if(bytes[1] == Start_FRAG){
		    packet_id = bytes[0];
		    LOGI("packet id : %d\n",packet_id);
		}
		memcpy(final_data+final_data_len, bytes + FRAGMENT_HEADER_SIZE, num_bytes - FRAGMENT_HEADER_SIZE);
                final_data_len += num_bytes - FRAGMENT_HEADER_SIZE;
                return true;
	    }
	    else {
	...
	final_data_len = num_bytes;
        memcpy(final_data, bytes, final_data_len);
    }
    ...
    r = sd_bus_message_append_array(m, 'y', final_data, final_data_len);
